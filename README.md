...
对 [Bangumi API](https://github.com/bangumi/api) v0 版本进行适配，支持漫画/轻小说主条目和单行本的元数据检索（参考 [calibre-web-bgm](https://github.com/JeffersonQin/calibre-web-bgm)），使用简单过滤算法对搜索结果进行筛选
...
- 官方提供的 v0 版本搜索接口处于实验阶段，筛选条件较少，且可能会导致搜索结果不准确，若无法找到某个条目的元数据，可以尝试在标识符中添加 `Bangumi ID` 来具体定位条目
